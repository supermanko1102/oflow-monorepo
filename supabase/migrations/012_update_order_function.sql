...
-- 版本：v2.0
-- 建立日期：2025-10-27
-- 說明：支援配送方式、冷凍配送、服務時長等新欄位
-- ═══════════════════════════════════════════════════════════════════

-- ───────────────────────────────────────────────────────────────────
-- 更新 create_order_from_ai 函數
-- ───────────────────────────────────────────────────────────────────

-- 先刪除所有舊版本的函數（避免函數重載衝突）
DROP FUNCTION IF EXISTS create_order_from_ai(UUID, TEXT, TEXT, JSONB, DECIMAL, DATE, TIME, UUID, TEXT, TEXT);
DROP FUNCTION IF EXISTS create_order_from_ai(UUID, TEXT, TEXT, JSONB, DECIMAL, DATE, TIME, UUID, TEXT, TEXT, UUID);
...
COMMENT ON FUNCTION create_order_from_ai IS '從 AI 解析結果建立訂單（v2.0），支援多行業、配送方式、服務型業務';
...
DO $$
BEGIN
  RAISE NOTICE '✅ 訂單建立函數已更新至 v2.0！';
  RAISE NOTICE '✅ 新增參數：';
  RAISE NOTICE '  - p_delivery_method (配送/服務方式)';
  RAISE NOTICE '  - p_requires_frozen (冷凍配送)';
  RAISE NOTICE '  - p_store_info (超商店號)';
  RAISE NOTICE '  - p_shipping_address (寄送地址)';
  RAISE NOTICE '  - p_service_duration (服務時長)';
  RAISE NOTICE '  - p_service_notes (服務備註)';
END $$;
